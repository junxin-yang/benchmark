...
from datasets import Dataset, TCGA_BRCA, CustomDataset
...
def main():
    ...
    for task_name, task_config in config.items():
        ...
        for model_name in task_models:
            ...
            for dataset_info in task_datasets:
                ...
                try:
                    ...
                    # [todo]
                    # initialize dataset
                    # dataset = Dataset(data_root=dataset_configs[dataset_name].get("root_dir"),
                    #                   dataset_name=dataset_name,
                    #                   method=model_name,
                    #                   supported_tasks=dataset_configs[dataset_name].get("supported_tasks"))
        ...
        for metric in task_metrics:
            ...
            #         bar_color="skyblue", bar_width=0.6, bar_gap=0.4, font_family="Liberation Serif", font_size=14)
            
            plot_bar(models=task_models, datasets=[d.get('name') for d in task_datasets], 
                    task_name=task_name, metric=metric, 
                    result_dir=result_dir, fig_dir=fig_dir, 
                    bar_color="orange", bar_width=0.4, bar_gap=0.6, font_family="Nimbus Roman", font_size=16)
